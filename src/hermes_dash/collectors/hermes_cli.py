...
def parse_status_output(output: str) -> HermesStatus:
    """Parse the full hermes status output."""
    status = HermesStatus(raw_output=output)
...
_cached_status: HermesStatus | None = None
_cached_at: float = 0.0
_CACHE_TTL: float = 10.0  # Cache for 10 seconds


def get_hermes_status(timeout: int = 5) -> HermesStatus | None:
...
def status_to_gateway_data(status: HermesStatus) -> dict[str, Any]:
...
def status_to_sessions_data(status: HermesStatus) -> dict[str, Any]:
