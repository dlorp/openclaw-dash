...
# Default repos - can be overridden in config file at ~/.config/hermes-dash/config.json
...
def _load_repos_from_config() -> list[str]:
    ...
    config_path = Path.home() / ".config" / "hermes-dash" / "config.json"
...
def collect_security_vulnerabilities() -> list[Alert]:
    ...
    try:
        from hermes_dash.security import run_audit
