...
class SecurityAudit:
    ...
    def __init__(self, hermes_dir: Path | str | None = None):
        self.hermes_dir = Path(hermes_dir) if hermes_dir else Path.home() / ".hermes"
    ...
    def scan_secrets(self, deep: bool = False) -> None:
        ...
        config_files = [
            self.hermes_dir / "config.json",
            self.hermes_dir / "config.yaml",
            self.hermes_dir / "config.yml",
            self.hermes_dir / ".env",
            self.hermes_dir / "credentials.json",
        ]

        # Add workspace files in deep mode
        if deep:
            workspace = self.hermes_dir / "workspace"
            if workspace.exists():
    ...
    def check_permissions(self) -> None:
        ...
        if not self.hermes_dir.exists():
            self.result.findings.append(
                Finding(
                    ...
                    description=f"{self.hermes_dir} does not exist",
                    path=str(self.hermes_dir),
                )
            )
            return
            ...
            if dir_mode & 0o077:  # Others or group have any access
                ...
                self.result.findings.append(
                    Finding(
                        ...
                        path=str(self.hermes_dir),
                        recommendation=f"chmod 700 {self.hermes_dir}",
                        auto_fixable=True,
                    )
                )
        ...
        for filename in sensitive_files:
            filepath = self.hermes_dir / filename
...
def run_audit(deep: bool = False, hermes_dir: Path | None = None) -> AuditResult:
    """Convenience function to run a security audit."""
    audit = SecurityAudit(hermes_dir=hermes_dir)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: python -m hermes_dash.security.audit --path ~/.hermes --deep",
    )
    parser.add_argument(
        "--path",
        "-p",
    ...
    result = run_audit(deep=args.deep, hermes_dir=target_path)
