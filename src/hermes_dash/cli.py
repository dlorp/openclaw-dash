...
GATEWAY_UNREACHABLE_MSG = """WARNING  Hermes Agent gateway not responding at localhost:18789
    ...
    Try:
      ...
      • hermes-dash --skip-gateway  (run without gateway)
"""
...
def get_status() -> dict[str, Any]:
    ...
    from hermes_dash.collectors import activity, cron, gateway, repos, sessions
    from hermes_dash.offline import format_gateway_error_short, is_offline_mode
...
def get_metrics() -> dict[str, Any]:
    """Collect all metrics."""
    from hermes_dash.metrics import CostTracker, GitHubMetrics, PerformanceMetrics
...
def print_status_text(status: dict[str, Any]) -> None:
    ...
    if degraded_collectors:
        warning_msg = (
            f"⚠️  Warning: Using fallback/cached data for: {', '.join(degraded_collectors)}"
        )
        console.print(f"\n[yellow]{warning_msg}[/]\n")
...
def run_security_audit(deep: bool = False, fix: bool = False, json_output: bool = False) -> int:
    ...
    from hermes_dash.security import DependencyScanner, SecurityAudit, SecurityFixer
...
def cmd_sinks(args: argparse.Namespace) -> int:
    ...
    """
    from hermes_dash.sinks.manager import _load_sink_config
    ...
    if not configs:
        console.print("[dim]No sinks configured in ~/.config/hermes-dash/config.toml[/]")
        console.print()
    ...
    try:
        ...
        console.print("[green]paho-mqtt installed[/] (pip install hermes-dash[mqtt])")
    except ImportError:
        console.print("[red]paho-mqtt NOT installed[/] — run: pip install hermes-dash[mqtt]")
...
def cmd_auto_merge(args: argparse.Namespace) -> int:
    ...
    repos = ["synapse-engine", "r3LAY", "t3rra1n", "hermes-dash"]
    if args.repo:
        repos = [args.repo]
...
def cmd_auto_backup(args: argparse.Namespace) -> int:
    ...
    Returns:
        Exit code (0 for success).
    """
    from hermes_dash.automation.backup import BackupVerifier, format_backup_report
...
def cmd_collectors(args: argparse.Namespace) -> int:
    ...
    Returns:
        Exit code (0 for success).
    """
    from hermes_dash.demo import is_demo_mode
    from hermes_dash.offline import is_offline_mode
    ...
    from hermes_dash.collectors import (
        activity,
        cron,
        gateway,
        get_cache,
...
def cmd_models(args: argparse.Namespace) -> int:
    ...
    Returns:
        Exit code (0 for success).
    """
    from hermes_dash.services import discover_local_models
...
def main() -> int:
    """Main entry point for hermes-dash CLI.
    ...
    parser.add_argument(
        "--version", action="version", version=f"%(prog)s {__import__('hermes_dash').__version__}"
    )
    parser.add_argument(
        "--demo",
        action="store_true",
    ...
    if args.skip_gateway:
        from hermes_dash.offline import enable_offline_mode

        enable_offline_mode()

    # Bare mode: minimal HDLS-only config
    if args.bare:
        from hermes_dash.config import set_bare_mode
    ...
    if args.command == "export":
        ...
        from hermes_dash.exporter import export_to_file
