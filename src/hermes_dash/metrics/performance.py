...
from hermes_dash.demo import is_demo_mode, mock_metrics

DEFAULT_METRICS_DIR = Path.home() / ".hermes" / "workspace" / "metrics"
GATEWAY_LOG_DIR = Path.home() / ".hermes" / "logs"
TMP_LOG_DIR = Path("/tmp/hermes")
...
class PerformanceMetrics:
    ...
    def _find_log_files(self) -> list[Path]:
        ...
        if TMP_LOG_DIR.exists():
            logs.extend(TMP_LOG_DIR.glob("hermes-*.log"))
