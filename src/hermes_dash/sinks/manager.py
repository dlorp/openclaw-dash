...
from hermes_dash.collectors import alerts, gateway, resources
...
def _load_sink_config() -> dict[str, dict[str, Any]]:
    ...
    config_path = Path.home() / ".config" / "hermes-dash" / "config.toml"
...
class SinkManager:
    ...
    def _create_mqtt_sink(cfg: dict[str, Any]) -> Any:
        ...
        """
        try:
            from hermes_dash.sinks.mqtt_sink import MqttSink
