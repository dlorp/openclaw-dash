...
def run_pip_audit(repo_path: Path, requirements_file: Path) -> list[Vulnerability]:
    ...
    code, stdout, stderr = run(
        ["pip-audit", "-r", str(requirements_file), "--format", "json"], cwd=repo_path, timeout=180
    )
...
def run_safety_check(repo_path: Path, requirements_file: Path) -> list[Vulnerability]:
    ...
    vulnerabilities = []
    code, stdout, stderr = run(
        ["safety", "check", "-r", str(requirements_file), "--json"], cwd=repo_path, timeout=180
    )
...
def get_pip_outdated(repo_path: Path) -> list[OutdatedDep]:
    ...
    code, stdout, stderr = run(["pip", "list", "--outdated", "--format", "json"], cwd=repo_path)
...
def run_npm_audit(package_dir: Path) -> list[Vulnerability]:
    ...
    code, stdout, stderr = run(["npm", "audit", "--json"], cwd=package_dir, timeout=180)
...
def get_npm_outdated(package_dir: Path) -> list[OutdatedDep]:
    ...
    code, stdout, stderr = run(["npm", "outdated", "--json"], cwd=package_dir)
