...
from tool_config import get_repo_path, get_repos, load_config
...
def scan_directory(path: Path, skip_dirs: set = None) -> list[dict]:
    ...
    skip_dirs = skip_dirs or {".git", "node_modules", "__pycache__", ".venv", "venv", "dist", "build"}
...
def format_results(results: dict, verbose: bool = False) -> str:
    ...
    if verbose:
        ...
        for issue_type, issues in by_type.items():
            ...
            for issue in issues:
                if "file" in issue:
                    lines.append(f"- **{issue['file']}:{issue.get('line', '?')}** - {issue['description']}")
                elif "package" in issue:
                    lines.append(f"- **{issue['package']}** ({issue.get('severity', 'unknown')}): {issue.get('description', '')}")
                else:
                    lines.append(f"- {issue.get('message', issue.get('description', 'Unknown issue'))}")
            lines.append("")
  ...
  %(prog)s --verbose            Show detailed findings
    ...
    if args.all:
        for repo in get_repos(config):
            p = get_repo_path(repo, config)
            if p.exists():
                paths.append(p)
    elif args.path:
        paths.append(Path(args.path))
    elif args.repo:
        paths.append(get_repo_path(args.repo, config))
    else:
