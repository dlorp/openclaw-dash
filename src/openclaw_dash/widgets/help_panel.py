...
def _categorize_binding(action: str) -> str:
    ...
    # Panel focus actions
    if "focus" in action_lower or action_lower in ("gateway", "security", "metrics", "alerts", "cron", "repos"):
        return "Panel Focus"
...
def extract_bindings_from_app(app: App) -> list[tuple[str, list[tuple[str, str]]]]:
    ...
    result = [
        ...
        for name, shortcuts in sorted(
            categories.items(),
            key=lambda x: (category_order.get(x[0], 50), x[0])
        )
    ]
                ...
                with VerticalScroll(id="help-scroll"):
                    for section_name, section_shortcuts in all_shortcuts:
                        ...
                        for key, desc in section_shortcuts:
                            yield Static(
                                f"  [bold yellow]{key:14}[/] {desc}",
                                classes="shortcut-row"
                            )

                yield Static("Press [bold]?[/], [bold]H[/], or [bold]Esc[/] to close", id="help-footer")
