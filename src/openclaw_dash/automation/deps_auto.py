...
def run(cmd: str, cwd: Optional[Path] = None, timeout: int = 300) -> tuple[int, str, str]:
...
class DepsAutomation:
    ...
    def __init__(self, config: Optional[DepsConfig] = None):
    ...
    def find_dep_shepherd(self) -> Optional[Path]:
        """Find the dep-shepherd script."""
        # Check common locations
        candidates = [
            Path(__file__).parent.parent / "tools" / "dep-shepherd.py",
            Path.home() / "repos" / "openclaw-dash" / "src" / "openclaw_dash" / "tools" / "dep-shepherd.py",
        ]
    ...
    def scan_repos(self) -> list[dict]:
        ...
        for repo in self.config.repos:
            code, stdout, stderr = run(
                f"{sys.executable} {dep_shepherd} --repo {repo} --json",
                timeout=300
            )
    ...
    def _update_npm_dep(self, repo_path: Path, package: str, version: str) -> bool:
        ...
        code, _, stderr = run(
            f"npm install {package}@{version} --save",
            cwd=repo_path,
            timeout=180
        )
        return code == 0
    ...
    def _generate_pr_body(self, package: str, current: str, latest: str, 
                          is_security: bool, test_output: str) -> str:
...
"""
    ...
    def run_updates(self, force: bool = False) -> list[UpdateResult]:
        ...
        } for r in results if r.status in ("created", "failed")])
...
def format_deps_results(results: list[UpdateResult]) -> str:
    ...
    lines.append(f"**Summary:** {len(created)} created, {len(dry_run)} dry-run, {len(failed)} failed, {len(skipped)} skipped")
