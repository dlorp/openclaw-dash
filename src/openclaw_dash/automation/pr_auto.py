...
class CleanupConfig:
    ...
    max_age_days: int = 30
    protect_patterns: list[str] = field(default_factory=lambda: ["main", "master", "develop", "release/*"])
...
def run(cmd: str, cwd: Optional[Path] = None, timeout: int = 60) -> tuple[int, str, str]:
...
class PRAutomation:
    ...
    def get_open_prs(self) -> list[PRInfo]:
        ...
        for pr_data in json.loads(stdout or "[]"):
            ...
            prs.append(PRInfo(
                ...
                ci_status=ci_status,
                approvals=approvals,
                labels=[l.get("name", "") for l in (pr_data.get("labels") or [])],
    ...
    def get_remote_branches(self) -> list[BranchInfo]:
        ...
        for line in stdout.split("\n"):
            ...
            try:
                commit_date = datetime.fromisoformat(parts[1].strip().replace(" ", "T").split("+")[0])
            except ValueError:
    ...
    def cleanup_branches(self, config: CleanupConfig) -> list[dict]:
        ...
        for branch in branches:
            ...
            if config.dry_run:
                ...
                code, stdout, stderr = run(
                    f"git push origin --delete {branch.name}",
                    cwd=self.repo_path
                )
...
def format_merge_results(results: list[dict], repo_name: str) -> str:
    ...
    lines.append(f"**Summary:** {len(merged)} merged, {len(would_merge)} would-merge, {len(skipped)} skipped, {len(failed)} failed")
